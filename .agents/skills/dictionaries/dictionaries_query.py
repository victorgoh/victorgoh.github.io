...
# Load dictionaries_dict.py dynamically
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
dict_file = os.path.join(SCRIPT_DIR, 'data', 'dictionaries_dict.py')
DICTIONARIES = {}
...
try:
    ...
    DICTIONARIES = local_vars.get('DICTIONARIES', {})
except Exception as e:
    print(f"Error loading dictionary database dictionary: {e}", file=sys.stderr)
    sys.exit(1)
...
def get_matches(query, dict_keys):
    ...
    # 4. Word-based flexible search for multi-word queries (e.g. "ark of covenen")
...
def find_db_path():
    ...
    p = os.path.join(home, 'biblemate', 'data', 'data', 'dictionary.data')
    if os.path.exists(p):
        return p
    # 2. Check workspace relative
    repo_root = os.path.abspath(os.path.join(SCRIPT_DIR, '..', '..', '..'))
    p2 = os.path.join(repo_root, 'biblemate', 'data', 'data', 'dictionary.data')
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 dictionaries_query.py <dictionary_entry>", file=sys.stderr)
        sys.exit(1)
        
    query = " ".join(sys.argv[1:])
    matches = get_matches(query, DICTIONARIES.keys())
    
    if not matches:
        print(f"Error: Could not locate any matching dictionary entries for '{query}'.", file=sys.stderr)
        sys.exit(1)
        
    db_path = find_db_path()
    if not db_path:
        print("Error: dictionary.data database file not found.", file=sys.stderr)
        sys.exit(1)
        
    md = []
    if len(matches) == 1:
        md.append(f"# Dictionary Entry: {clean_key(matches[0])}")
    else:
        md.append(f"# Dictionary Search Results for '{query}' (Found {len(matches)} matches)")
    ...
    try:
        ...
        for m_idx, match in enumerate(matches):
            paths = DICTIONARIES[match]
            if len(matches) > 1:
                md.append(f"\n# Entry: {clean_key(match)}")
                
            for p_idx, path in enumerate(paths):
                c.execute("SELECT content FROM Dictionary WHERE path = ?", (path,))
                row = c.fetchone()
                
                if not row or not row[0]:
                    content_md = f"*(No database record content found for path `{path}`)*"
                else:
    ...
    filename = f"{timestamp}_dictionary_{clean_name}.md"
